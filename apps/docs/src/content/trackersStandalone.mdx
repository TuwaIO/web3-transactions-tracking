...
While the `@tuwa/web3-transactions-tracking` library provides a comprehensive solution with a UI (`@tuwa/transactions-tracking-ui`) and a Zustand-based state store (`@tuwa/evm-transactions-tracking`), its architecture is modular. You can use the low-level **trackers** (`evmTracker`, `gelatoTracker`, `safeTracker`) directly, without installing or configuring the state management store.
...
```tsx
import { evmTracker } from '@tuwa/evm-transactions-tracking';
...
```tsx
import { gelatoTracker, GelatoTaskState } from '@tuwa/evm-transactions-tracking';
...
```tsx
import { safeTracker } from '@tuwa/evm-transactions-tracking';
...
```tsx
import { checkTransactionsTracker, TransactionTracker, isGelatoTxKey } from '@tuwa/evm-transactions-tracking';
...
```tsx
import { checkChainForTx } from '@tuwa/evm-transactions-tracking';
